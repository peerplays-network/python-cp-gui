        ...
        while True:
            try:
                ...
                if len(eventsAll) == 0: 
                    return eventsAll
                eventsAll = eventsAll.sort_values("start_time")
                # eventsAll.to_csv("events_all_sorted.csv")
                eventsAll.to_pickle("events_all_sorted.pkl")

                # print(time.time() - tic)
                return eventsAll
            except:
                # eventsAll = pd.read_csv("events_all_sorted.csv")
                eventsAll = pd.read_pickle("events_all_sorted.pkl")
                return eventsAll

    def EventsAllSortedForApi(self):
        print('Fetching all active events, wait a few seconds')
        # eventsAllRaw = node.getEvents("all")
        eventsAll = self.GetEventsAll()
