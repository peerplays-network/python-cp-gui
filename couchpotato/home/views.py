...
def getstaticEvents():
    ...
    ''' ##  Dummy line for VIM Python IDE syntax highlighting fix required due to above long line'''
    return newDict
...
def LoadSelectOptions(request):
    '''
    param-1 : request
    param-2 : The values needed are posted from create_cp html page
    num - represents game , group , home , away and time in integer representation 
    '''
    ...
    if index_page_permitted(request):
        ...
        if num  == 2:
            ...
            return JsonResponse({"list" :list_values,"islast":is_last,'question_id':num })
        if num == 3:
            request.session['sport'] = request.POST.get("sport").strip()
        ...
        if num == 5:
            heading = "Time"
            is_last = True
            request.session['away'] = request.POST.get('away').strip()
            return JsonResponse({"list" :list_values,"islast":is_last,'question_id':num })
